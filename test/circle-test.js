...
it("circle.center([0, 90])", () => {
  const o = d3.geoCircle().center([0, 90])();
  assert.strictEqual(o.type, "Polygon");
  assertInDelta(o.coordinates, [array.range(360, -1, -6).map(function(x) { return [x >= 180 ? x - 360 : x, 0]; })], 1e-6);
});

it("circle.center([45, 45])", () => {
  const o = d3.geoCircle().center([45, 45]).radius(0)();
  assert.strictEqual(o.type, "Polygon");
  assertInDelta(o.coordinates[0][0], [45, 45], 1e-6);
});

it("circle: first and last points are coincident", () => {
  const o = d3.geoCircle().center([0, 0]).radius(0.02).precision(45)();
